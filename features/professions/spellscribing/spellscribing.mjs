...
export default {
    register() {
        ...
        TaliaCustomAPI.add({spellScribing: showUI}, "ItemMacros");
    }
}
 ...
 */
async function spellscribing(actor, chosenArgs) {
    ...
    //consume gem item
    await TaliaUtils.Helpers.consumeItem(chosenArgs.chosenGem, 1);
...
async function causeSurges(actor, chosenArgs) {   //do this later as it requires reworking the WMS stuff
    ...
    for(let i = 0; i < amount; i++) {
        await TaliaCustom.Macros.wildMagicSurge(actor, {allowedSeverities});
    }
}
