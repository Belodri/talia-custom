...
export default {
    register() {
        ...
        TaliaCustomAPI.add({
            cunningContraptions: openContraptionCrafting,
            triggerContraption
        }, "ItemMacros");
    }
}
...
class ContraptionCraftingUI extends dnd5e.applications.DialogMixin(FormApplication) {
    ...
    async craftContraption() {
        ...
        const command = `await TaliaCustom.ItemMacros.triggerContraption(item, actor, token);`;
...
async function triggerContraption(item, actor, token) {
    ...
    if(!await TaliaUtils.Helpers.promptItemUse(item)) return;
